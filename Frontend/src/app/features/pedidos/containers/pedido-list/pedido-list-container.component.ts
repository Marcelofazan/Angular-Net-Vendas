// frontend/src/app/features/pedidos/containers/pedido-list/pedido-list-container.component.ts
...
import { PagedResult, PedidoListItemDto, PedidoStatus } from '../../../../shared/models/api-models';
import { PedidoTableComponent } from '../../../../shared/components/pedido-table/pedido-table.component';
import { PaginationComponent } from '../../../../shared/components/pagination/pagination.component';
import { Router } from '@angular/router';
import { FeedbackService } from '../../../../core/services/feedback.service';

@Component({
  selector: 'app-pedido-list-container',
  standalone: true,
  imports: [CommonModule, ReactiveFormsModule, PedidoTableComponent, PaginationComponent],
  template: `
    <div class="pedido-list-container">
      ...
      <div class="filters">
        <div class="filter-group">
          ...
          <input 
            id="pessoaNome"
            type="text" 
            [formControl]="pessoaNomeFilterControl" 
      ...
      <div *ngIf="!isLoading() && pagedResult()">
        <app-pedido-table
          [pedidos]="pagedResult()?.items || []"
          (view)="viewPedido($event)">
        </app-pedido-table>
...
export class PedidoListContainerComponent implements OnInit {

  // --- Estado da Aplicação (Signals) ---
  pagedResult = signal<PagedResult<PedidoListItemDto> | null>(null);
  ...
  pessoaNomeFilterControl = new FormControl(''); 
  
  // Controla o filtro por status (CRIADO, PAGO, etc.)
  statusFilterControl = new FormControl<'ALL' | PedidoStatus>('ALL'); 
  
  // Opções de Status (baseado na enumeração do backend)
  statusOptions: Array<'ALL' | PedidoStatus> = ['ALL', 'CRIADO', 'PAGO', 'CANCELADO'];

  constructor(private pedidoService: PedidoService, private router: Router, private feedback: FeedbackService) {}

  ngOnInit(): void {
    
    // 1. Define os fluxos reativos de filtros
    const nomeFilter$ = this.pessoaNomeFilterControl.valueChanges.pipe(
      debounceTime(400),
    ...
    combineLatest([
      ...
      nomeFilter$.pipe(startWith('')),
      statusFilter$.pipe(startWith('ALL'))
    ])
    .pipe(
        // Troca para o Observable de chamada da API
        switchMap(([page, nomeFilter, statusFilter]) => {
            this.isLoading.set(true); 
            
            // Mapeia 'ALL' para nulo, permitindo ao backend ignorar o filtro
            const finalStatus = statusFilter === 'ALL' ? null : statusFilter as PedidoStatus;
            
            // Chama o PedidoService (Endpoint GET /api/pedidos)
  ...
  viewPedido(pedidoId: string): void {
